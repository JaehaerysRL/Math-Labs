...
\begin{sloppypar}
    ...
    \end{itemize}
    \emphb{算术表达式(Arithmetic Expressions)}: 以算术表达式为例，定义了基本的数字和变量作为基础元素，以及加法、乘法等操作作为构造规则。通过结构归纳法，可以证明诸如“所有算术表达式的值都是整数”或“表达式的深度不超过某个值”等性质。

    \subsection{无限集合 (Infinite Sets)}
    \emphb{康托尔(Cantor)的无限基数理论}: 康托尔通过对集合的基数进行比较，定义了不同类型的无限集合。通过构造单射、满射和双射来比较集合的大小，证明了即使是无限集合，也有大小之分(例如，实数集“严格大于”整数集)。\\
    \emphb{关于逻辑公式的两个元定理(Meta-theorems)}:
    \begin{itemize}[nosep]
    ...
    \emphb{ZFC 集合论}: ZFC 集合论是由 Zermelo-Fraenkel 公理系统加上选择公理(Axiom of Choice)组成的集合论体系。它通过一系列严格的公理来定义集合的存在和操作，避免了罗素悖论等逻辑矛盾，使得集合论成为一个坚实的数学基础。
    \begin{itemize}[nosep]
